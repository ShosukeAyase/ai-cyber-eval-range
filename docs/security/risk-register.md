...
| R-026 | Model output bypasses Tool Gateway through a hidden execution path | 2 | 5 | 10 | No provider tools, closed proposal schema, architecture import checks, mandatory internal Tool Gateway invocation | AI Security | Reassess for every new model or tool adapter |
| R-027 | Provider credential or secret context reaches the model | 2 | 5 | 10 | Secret-reference redaction, transport-only authentication, prompt serialization tests, secret scan | IAM | Move provider authentication to KMS-backed broker before production |
